...
def create_defect_files(file_path, max_iterations=5, delta=5.0, supercell_size=29.47839554):
    ...
    for iteration in range(1, max_iterations + 1):
        if not cube_o_core_atoms:
            print(f"Stopping at iteration {iteration}, not enough oxygen atoms left in the cube.")
            break

        # Find the closest O core atom to the most central Mg
        closest_o_core = find_closest_atom(most_central_mg, cube_o_core_atoms)
        closest_o_core_coords = list(map(float, closest_o_core.split()[2:5]))

        # Determine row index of the closest O core
        o_core_index = o_core_atoms.index(closest_o_core)

        # Find the corresponding O shel based on the row index
        try:
            closest_o_shel = o_shel_atoms[o_core_index]
        except IndexError:
            print(f"Error: No matching O shel found for O core at row {o_core_index}. Skipping iteration.")
            continue

        print(f"Iteration {iteration}:")
        print(f"  Closest O core: {closest_o_core.strip()} with coordinates: {closest_o_core_coords}")
        print(f"  Closest O shel: {closest_o_shel.strip()} with coordinates: {list(map(float, closest_o_shel.split()[2:5]))}")
