...
def create_defect_files(file_path, max_iterations=5, delta=5.0, supercell_size=29.47839554):
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        # Match the O shel atom with the same coordinates as the O core
        closest_o_shel = next(
            atom for atom in cube_o_shel_atoms
            if list(map(float, atom.split()[2:5])) == closest_o_core_coords
        )

        print(f"Iteration {iteration}:")
        print(f"  Closest O core: {closest_o_core.strip()} with coordinates: {closest_o_core_coords}")
        print(f"  Closest O shel: {closest_o_shel.strip()} with coordinates: {closest_o_core_coords}")
